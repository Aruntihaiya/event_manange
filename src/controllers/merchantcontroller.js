...
const MerchantRepo = require("../dbRepository/queryhelper/MerchantRepo");
// const merchantuser = require("../models/merchantuser");

// const user = db.merchantuser;

const merchantuser = {
  async merchntregister(req, res) {
    const data = await MerchantRepo.merchntregisterdata(req.body);
  ...
  async merchntloginuser(req, res) {
    const data = await MerchantRepo.merchntloginuserdata(req.body);
  ...
  async merchntforgetpass(req, res) {
    const data = await MerchantRepo.merchntforgetpassdata(req.body);
  ...
  async merchntresetpassword(req, res) {
    console.log(req.body, "xyz");
    const data = await MerchantRepo.merchntresetpassworddata(req.body);
  ...
  async userprofile(req, res) {
    console.log(req.body, "xyz");
    const data = await MerchantRepo.userprofiledata(req.params);
  ...
  async updateprofile(req, res) {
    ...
    const data = await MerchantRepo.updateprofiledata(
      req.body,
      profileimage
    );
    if (data && data.action == "profile data") {
  ...
  async changepassword(req, res) {
    console.log(req.body, "xyz");
    const data = await MerchantRepo.changepassworddata(req.params);
    if (data && data.action == "not match") {
      res.status(400).json({ message: 'New password and confirm password do not match', success: true });
